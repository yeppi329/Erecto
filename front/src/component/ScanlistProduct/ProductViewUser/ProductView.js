...
function VocView() {
  ...
  const [category, setCategory] = useState(""); //얘는 수정할 카테고리

  const body = {
    label: category, //얘는 수정할 카테고리
  };
  ...
  return (
    <Box className="contents" sx={{ display: "flex" }}>
      ...
      <Container maxWidth="lg" sx={{ mt: 8, mb: 4 }}>
        <ProductViewModal checkedItem={checkedItem} />
        <Paper
          sx={{
            display: "flex",
            ...
            {Object.values(data).map((img) =>
              img.view === "on" ? (
                ...
                >
                  ...
                  <img src={img.src} alt="img" height="auto" width="100%" />
                  <FormControlLabel
                    value={img.src}
                    control={
                      <Checkbox
